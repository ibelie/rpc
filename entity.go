...
%s`, object.Name, object.Name, object.Name, object.Name, strings.Join(methods, ""))))
	...
	symbol_c, _, _, _ := entitySymbols(symbols)
	body.Write([]byte(fmt.Sprintf(`
const (%s
)`, strings.Join(symbol_c, ""))))
...
func entityProxy(identName string, dir string, entities []*Entity) ([]string, []byte) {
	...
	symbol_c, symbol_v, symbol_s, symbol_b := entitySymbols(symbols)
...
%s
`, ident[0], ident[1], strings.Join(symbol_c, ""), strings.Join(versionBytes, ", "), strings.Join(symbol_v, ""), entityRoutes(entities))), 0666)

	return symbol_s, symbol_b
}
...
func entitySymbols(symbols []string) ([]string, []string, []string, []byte) {
	maxSymbol := 0
	...
	return symbolConst, symbolValue, append(BUILTIN_SYMBOLS, symbolSorted...), version.Sum(nil)
}
